...
function generaListCorresFULL(R, Tip) {
        ...
        if (R[i].datCorres.CORRESID_CONTESTA == -1) {
            ...
            if (R[i].datCorres.CATEGOID == 1) {
                ...
                contenido = ""
                    ...
                    + "<span id='icoAdjun' class='mif-attachment place-right fg-darkBlue' style='display:" + tieneAdjunto + ";'></span>"
    ...
    /*===========================Con dependencia=============================*/
    /*======================================================================*/
    var RC = _.filter(R, function (c) { return c.datCorres.CATEGOID == 1 && c.datCorres.CORRESID_CONTESTA != -1 })
    $.each(RC, function (i, e) {
        ico = "", posee = "", nombre = "";
        //get key,ico,posee..
        keys = getIcoKeyPosee(e);
        //get poseción
        posee = definirPosecion(e, Tip);
        //get ico
        ico = getIco(e);

        //ad class inactivo
        clases = "";
        if (e.datCorres.ACTIVO == 2) { clases += "inactivo" }

        //Limita longitud de caracteres del Asunto
        var asunto = e.datCorres.ASUNTO;
        if (asunto.length > 100) {
            asunto = asunto.substring(0, 100) + "...";
        }

        /*****************CORRESPONDENCIA con dependencia**************/


        //Nombre remite
        if (e.datCorres.CONTNOMBRE_REMITE == "") { nombre = e.datCorres.CONTUNIDAD_REMITE }
        else { nombre = e.datCorres.CONTNOMBRE_REMITE }

        //Si tiene Adjuntos
        var tieneAdjunto = "";
        if (e.datCorres.ADJUNTO == 1)
            tieneAdjunto = "<span class='mif-attachment place-right fg-darkBlue'></span><p style='display:none;'>" + keys + "</p> "

        contenido = ""
                    + "<div id=\"corres_" + e.datCorres.CORRESID + "\" class='list margi idPadre_" + e.datCorres.CORRESID_CONTESTA + " " + clases + "'>"
					 + "<b class='place-right' style='font-size:10px;'>Id:" + e.datCorres.CORRESID + "</b><br />"
                    + tieneAdjunto
                    + " <p class='place-right'>" + posee + "</p>"
                    + "	<span class='icon " + ico + "'></span><p style='display:none;'>" + keys + "</p> "
					+ "	<b class='item-remitente'>" + e.datCorres.CONTINST_REMITE + "</b>"
					+ "	<p class=''>" + nombre + "</p>"

					+ "	<p class='item-asunto'>" + asunto + "</p>"
					+ "	<p class='item-fecha'>" + (($("#cbOrdenar").val() == 1) ? e.datCorres.NOTAINSERT :e.datCorres.NOTARECIBFEC) + "</p>"
					+ "	<p class='item-estado'>Estado: " + getEstadoList(e) + "</p>"//CORRESDETALLE.js
					+ "</div>";

        $("#corres_" + e.datCorres.CORRESID_CONTESTA).after(contenido);
        //Carga data
        cargaData(e);
    });
    ...
    var RM = _.filter(R, function (c) { return c.datCorres.CATEGOID == 2 && c.datCorres.CORRESID_CONTESTA != -1 })
    $.each(RM, function (i, e) {
        ...
        var datMargiRes = _.filter(R, function (c) { return c.datCorres.CORRESID == e.datCorres.CORRESID_CONTESTA })[0].datmargi
                for (var j = 0; j < datMargiRes.length; j++) {
                    if (datMargiRes[j].MARGID == e.datCorres.MARGINAID) {
                        ...
                        //Limita longitud de caracteres del Asunto
                        var forma = e.datCorres.MARGINAFORMARES;
                        if (forma.length > 100) {
    ...
    var RN = _.filter(R, function (c) { return c.datCorres.CATEGOID == 3 && c.datCorres.CORRESID_CONTESTA != -1 })
    $.each(RN, function (i, e) {
        ico = "", posee = "", nombre = "";
        //get key,ico,posee..
        keys = getIcoKeyPosee(e);
        //get poseción
        ...
        /*****************NOTAS con dependencia**************/

        //Nombre dirigido
        if (e.datCorres.CONTNOMBRE_DIRIGIDO == "") { nombre = e.datCorres.CONTUNIDAD_DIRIGIDO }
        else { nombre = e.datCorres.CONTNOMBRE_DIRIGIDO }
        ...
        contenido = ""
                    ...
                    + " <p class='place-right'>" + posee + "</p>"
                    + "	<span class='icon " + ico + "'></span><p style='display:none;'>" + keys + "</p> "
					+ "	<b class='item-remitente'>" + e.datCorres.CONTINST_DIRIGIDO + "</b>"
					+ "	<p class=''>" + nombre + "</p>"

					+ "	<p class='item-asunto'>" + asunto + "</p>"
					+ "	<p class='item-fecha'>" + (($("#cbOrdenar").val() == 1) ? e.datCorres.NOTAINSERT :e.datCorres.NOTAELABFEC) + "</p>"
					+ "	<p class='item-estado'>Estado: " + getEstadoList(e) + "</p>"//CORRESDETALLE.js
					+ "</div>";
