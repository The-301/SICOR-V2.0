...
function cargaUpFile() {
    ...
    if (item.length == 0) {
        ...
        if ($("#detalle-adjunto").css('display') == "none") {
            ...
            $("#detalle-adjunto").load("UPFILE/Default.aspx").fadeIn(500);
        } else {
...
function adjuntoSV(datAdjunto) {    
    var itemId = $("#lista-item-corres .list.active").data("datos").datCorres.CORRESID;
    var nombreArchivo = "IDOC_" + itemId + "-" + remover_acentos(datAdjunto.name)
    , name =  nombreArchivo
    ...
    , Datos = "Datos:{"
            ...
            + ",'corresid':'" + itemId + "'"
            + ",'adjunto_name':'" + name + "'"
            + ",'adjunto_size':'" + datAdjunto.size + "'"
            + ",'adjunto_type':'" + datAdjunto.type + "'"
            + ",'adjunto_urldown':''"
            + ",'adjunto_urldel':''"
            + ',"updateusrid":"' + $.jCookies({ get: 'USRID' }) + '"'
            + "}";
    ...
    $.when(sp_adjuntoSVjs(Datos,grupo)).then(function (response) {
        ...
        if (R > 0) {
            ...
            $("#corres_" + itemId + " #icoAdjun").css("display", "block");
            $("#corres_" + itemId + " .mif-attachment").css("display", "block");
            $("#lista-item-corres .list.active").data("datos").datCorres.ADJUNTO = 1;
            //agregamos el nombre del archivo para la lista
            var tamanioArch = ((parseFloat(datAdjunto.size) / 1024) < 1024) ? (parseFloat(datAdjunto.size) / 1024).toFixed(2) + " KB" : ((parseFloat(datAdjunto.size) / 1024) / 1024).toFixed(2) + " MB";
            $("#lista-item-corres .list.active").data("datos").datfile.push({ 'ADJUNTOID': R, 'CORRESID': itemId, 'ADJUNTO_SIZE': tamanioArch, 'ADJUNTO_NAME': nombreArchivo })
            $("#submenuitem #adjunto").click();
            setTimeout(function () {
                $("#submenuitem #adjunto").click();        
            }, 1000);
        } else { mError(R); }
...
/*                           Eliminar Adjunto                      */
/*-----------------------------------------------------------------*/
function adjuntoDL(adjuntoName) {//folder,
