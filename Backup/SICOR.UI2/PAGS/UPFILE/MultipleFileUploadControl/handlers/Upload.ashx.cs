...
namespace MultipleFileUpload.handlers
{
    public class Upload : IHttpHandler
    {
        ...
        public string StorageFolder2
        {
            get
            {
                return HttpContext.Current.Request.QueryString["storageFolder"].Substring(0, HttpContext.Current.Request.QueryString["storageFolder"].Length - 4) + "2";
            }
        }
        ...
        {//String[] new String[]{};
             FileStatus[] statuses =
                (from file in new DirectoryInfo(StorageFolder2).GetFiles("*", SearchOption.TopDirectoryOnly)
                 where !file.Attributes.HasFlag(FileAttributes.Hidden) && file.Name.Substring(0,6)!="Delete"
                 && file.Name.Substring(0, 6) == "TESTER"
                 select new FileStatus(file, StorageFolder2)).ToArray();
        ...
        private void DeleteFile(HttpContext context)
        {
            ...
            var newFilePath = Path.Combine(StorageFolder, "Delete["+fecha+"]_"+context.Request["f"]);
        ...
        private void UploadWholeFile(HttpContext context, List<FileStatus> statuses)
        {
            ...
            for (int i = 0; i < context.Request.Files.Count; i++)
            {
                ...
                string fileName = context.Request["title[]"]; //Path.GetFileName(file.FileName);
                FileStatus status = new FileStatus(fileName, file.ContentLength, StorageFolder);

                try
                {
                    string fullName = Path.Combine(StorageFolder, fileName);
